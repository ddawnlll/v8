...
| `v8-core/src/kaizen/iteration.rs` | D-145 real-tape economic Kaizen runner: per-asset canonical USD-M replay, append-only iteration receipts, configuration hashes, fixed baseline safety ceiling, frontier comparison, and fail-closed candidate validation | `EconomicIterationConfig`, `EconomicIterationRunner`, `EconomicIterationReceipt`, `EconomicFrontier`, `candidate_seed_set` | D-032, D-129, D-141, D-145; Constitution Rule 12 |
| `v8-core/src/bin/kaizen_iterations.rs` | D-145 executable driver that bootstraps the real-tape baseline, evaluates deterministic challengers, and writes a summary only from physical receipts | `main` CLI: tape, output root, accepted target | D-145; `research/tape/quad-1h-12m/tape.jsonl` |
