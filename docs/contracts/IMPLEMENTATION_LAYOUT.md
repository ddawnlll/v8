...
| `v8-core/attic/evaluation/` | W2 attic: 7 dead evaluation surfaces (`manifest`, `statistics`, `surfaces`, `paths`, `regression`, `html_report`, `deployment_case`); `EvaluationEngine` removed from `evaluation/mod.rs`, row carriers `BarRow`/`CandidateRow`/`TradeRow` retained for live consumers. Not compiled. | Inert source text | D-160 §§2, 4; Issue #345 |
