...
v8-core/
  src/
    ...
    parquet_artifact.rs physical standard Parquet adapter, atomic publication, and read-back verification (D-156)
...
| `v8-core/src/checkpoint.rs` | Atomic simulation state checkpoint & resume with version and tape-hash binding | `SimulationCheckpoint.save_to_file/load_from_file` | PERSISTENCE_REPLAY_SPEC §4; D-122, D-156 |
| `v8-core/src/parquet_artifact.rs` | Physical standard Parquet writer and verifier with typed nullable columns, canonical row provenance, atomic publication, and row-count read-back | `write_json_rows`, `verify_parquet`, `ParquetArtifactReceipt` | D-156; Issue #321; Rules 5, 12, 44 |
| `v8-core/src/evidence.rs` | V8.2-compatible physical evidence headers, run constants, tiers, retention resolution, and byte-stable artifact read-back | `Artifact`, `RunConstants`, `ArtifactTier`, `RetentionStore`, `read_artifact`, `validate_header` | LEDGER_FORMAT_SPEC; D-079, D-156; Issue #319 |
| `v8-core/src/candidate.rs` | Append-only candidate transition registry and hash-bound replay projection | `CandidateRegistry`, `TransitionRecord`, `replay_jsonl` | D-018, D-026, D-156; Issue #319 |
| `v8-core/src/cashflow.rs` | Physical cashflow records and conservation-checked append-only publication | `EconomicCashflow`, `CashflowLedger`, `verify_conservation` | D-129, D-131, D-156; Issue #319 |
| `v8-core/src/evaluation/statistics.rs` | Finite-input statistical kernel, null-model absence, bootstrap/permutation results, and explicitly labelled proxy statistics | `BootstrapResult`, `NullModelResult`, `PermutationResult`, `ProxyStatistic`, `compute_proxy_deflated_sharpe_ratio`, `compute_deflated_sharpe_ratio` | D-118, D-153, D-156; Issue #322; Rule 12 |
| `v8-core/src/evaluation/multiple_testing.rs` | Complete search-family lineage, trial conservation, optional genuine PBO/DSR fields, and no-claim status when no registered estimator has consumed the lineage | `ResearchMultiplicityLedger`, `MultipleTestingSummary` | D-153, D-156; Issue #322; Rule 12 |
| `v8-core/src/usdm_sim/scenario_ruin.rs` | Physical-trade resampling for scenario ruin and explicit unresolved SaR output without synthetic baseline inputs | `ScenarioRuinReport`, `SlippageAtRiskReport`, `run_scenario_ruin_simulation`, `build_baseline_scenario_ruin` | D-156; Issue #320; Rules 5, 12 |
| `v8-core/src/benchmark/runner.rs` | Physical evidence manifest validation and fail-closed boundary until a registered data-backed evaluator exists | `BenchmarkRunner::run_benchmark` | D-153, D-156; Issue #323; Rules 12, 44 |
| `v8-core/src/cache.rs` | Durable redb cache with versioned canonical identity, digest validation, transactional inserts, compaction, and guarded JSONL migration | `CacheStore`, `CacheEntry`, `canonical_key`, `key_digest` | D-156; Issue #324; Rules 5, 12 |
...
| `v8-core/attic/kaizen-governance/` | W1 attic: 9 dead kaizen-governance modules (net; `controller`+`verdict` reverted to live by the W1 amendment, §4). Not compiled. | Inert source text (history via `git log --follow`) | D-160 §2; Issue #344 |
| `v8-core/attic/evaluation/` | W2 attic: 7 dead evaluation surfaces (`manifest`, `statistics`, `surfaces`, `paths`, `regression`, `html_report`, `deployment_case`); `EvaluationEngine` removed from `v8-core/src/evaluation/mod.rs`, row carriers `BarRow`/`CandidateRow`/`TradeRow` retained for live consumers. Not compiled. | Inert source text | D-160 §§2, 4; Issue #345 |
| `v8-core/attic/w3-dead-quartet/` | W3 attic: `checkpoint`, `world/learned`, `analysis/scorecard`, `opportunity/harness_t1_t12` (+ `README.md` provenance table). Not compiled. | Inert source text | D-160 §2; Issue #346 |
...
| V8.6 Phase-1 attic program removes live-tree modules registered by earlier decisions (`kaizen/*` under D-132/PH2-003A, `evaluation/*` under D-147/D-149, `checkpoint.rs` under D-119..D-122) | `v8-core/attic/*` | **DOCUMENTED, not reversed** — D-160 §§2–4: each removal carries a zero-consumer receipt and a green verification gate; relocated citations classify as `RETIRED` under `tools/audit_doc_path_refs.py`. `kaizen/{controller,verdict}.rs` are explicitly NOT removed (W1 amendment: D-132/Rule 35 enforcement names them). |
