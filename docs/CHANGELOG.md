...
## 2026-09-12 — Superseded evidence over stored bytes: portfolio ledger `seq 20` retired, readiness battery re-taken, regression baseline left frozen with its correction recorded (D-166)

Three records over evidence already on disk, none of which a worker could take (the
canonical ledger is append-only and its stored bytes may not be hand-edited, and the
readiness regression baseline is guarded: "a regression is reported, never repaired by
moving the baseline: re-pinning requires an entry in the decision register").

Portfolio ledger `seq 20` (`entry_hash 313a5cd6…`, `receipt_digest fa004b38…`,
`case_id BC-QUAD-PORTFOLIO-01`) is retired as **superseded defect evidence**: minted by
the pre-fix default-argument call site in `app/portfolio.py`, its `g0/g1/g2 = PASS` cells
carry no boundary measurement and its `capability_score 14.0` is inherited from them,
while its bound receipt `economic_receipt_e45add52.json` publishes `concentration = 1.0`
on all ten curves. From this record on, those cells count toward no clause, no readiness
input and no capability surface. The entry stays in the ledger unmodified — the
supersession lives in the decision register, not in the bytes. The receipt the portfolio
producer path publishes now is `seq 21` (`g0 PASS / g1 UNKNOWN / g2 BLOCKED`,
`capability_score null`), bound **with its own declaration** that it is a smoke window
(`economic_evidence: false`, 336 bars), so the binding is not an evidential claim and the
next evidential-window re-take remains a named residual.

`docs/evidence/v87/READINESS/readiness_audit.json` is re-taken by its own producer
(`v8-next/tools/nx_readiness_audit.py`, the #447 contract) rather than retired: the
superseded revision (no `structural_gates` block, `g1/g2 PASS`, `passed 4`) is replaced by
a measurement that publishes each structural cell with its input and named reason —
`g1 UNKNOWN`, `g2 UNKNOWN`, `g4 PASS`, `passed 2`. Everything else in the artifact is
unchanged (window 744 bars / same tape, capability 12.0, `technical_delivery` 90.9,
`economic_readiness` 0.0). The readiness consumer that refused the superseded revision
(`REFUSED`, 0/10, `GATE_AUDIT_CONTRACT_MISMATCH`) now counts the producer's own
measurement. The readiness regression baseline is deliberately **not** re-pinned: the
published snapshot is a landed, test-guarded pre-#470 record (`#449`/`#470`'s own guard
asserts it still carries the cross-basis drawdown cell and that it "is read, never
re-pinned"), and a probed re-take would also have carried the #449 basis correction as a
side effect. The regression is therefore left **reported, not repaired**: `gate_passes`
`4 -> 2` — the pinned 4 counted two unmeasured g1/g2 cells as PASS and was never reachable
from the tree's own code — with every other metric `ok`. The probed snapshot is preserved
as a card attachment (`readiness_baseline_repin_probe.json`, sha256 `ffbad558…`) instead of
being landed. Marking the corrected count so it stops reporting as a regression is scoped
to card `t_7337280b` (#458 Target (3) stays open). No measured breach is removed from any
landed artifact (all five fee-drag breaches and the pre-#470 drawdown cell stay published).
No economic authority is created or transferred; every verdict remains `NO_ECONOMIC_CLAIM`.

Modified artifacts: `docs/decisions/DECISION_REGISTER.md`, `docs/tr/DECISION_REGISTER.md`,
`docs/evidence/v87/READINESS/readiness_audit.json` (regenerated by its own producer; the
regression baseline is left frozen by this entry),
`docs/CHANGELOG.md`; Issues #471, #472, #458; card `t_3b618d47`; D-152, D-153, D-162,
D-163, D-164, D-165.
