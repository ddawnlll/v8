...
- **#327 Gate vector / authority firewall:** `GateVector` canonicalized to the registered G0–G9 with a non-compensable `GateState::Missing` (never evaluated, never a pass), typed `GateFailureClass`, and a `ReadinessVerdict` that is a projection rather than a local computation. New `v8-core/src/benchmark/gate_authority.rs` enforces monotone non-escalation (`cap_authority`), routes every authority claim through `ClaimRegistry`, fails closed on absence, and ceilings `PolicyCertificate` status so a scalar `CapabilityScore` can no longer print `PRODUCTION_READY`. The D-152 §5 versus D-153 §2 G7–G9 naming conflict is carried as an explicit `OPEN_PIN` (`OPEN_PIN_GATE_NAMING`), not adjudicated. Verified by 15 tests in `v8-core/tests/d152_gate_vector_authority_firewall.rs`.
- **#328 Receipt and ledger cryptographic self-verification:** the ad-hoc receipt digest is replaced by a canonical `crate::hash::Canon` digest over the whole receipt, versioned as `d153.receipt.v2` with `d153.receipt.v1` recognised as legacy. Added `BenchmarkProvenance`, `ArtifactBinding`, `verify()` / `verify_artifacts()` / `verify_policy_identity()`, constant-time digest comparison, non-finite rejection, and report rendering gated on `VerifiedReceipt` with recomputed-digest stamping in JSON and HTML. The ledger now seals entries with `d153.ledger.v2`, folding canonical receipt bytes into the chain, and exposes `LedgerTamper`, stable tamper codes, and `LedgerAuditReport` via `audit()` / `load_with_report()`. Pre-v2 rows are honestly classified `legacy_unbound` (the repository's real 7-row ledger reports `LEDGER_PARTIALLY_BOUND` at exit `3`) and are neither deleted nor claimed verified. Verified by 40 tests in `v8-core/tests/d153_receipt_ledger_selfverify.rs`.
- **#329 Policy-bound external parity adapters:** deleted the fabricated parity surface (`CommodityExecutionAdapter`, `LeanParityAdapter`, `SkfolioParityAdapter`, `VectorBtParityAdapter`, `ExecutionParityReport`, `evaluate_parity`, `parity_passed`) including its hardcoded comparison arrays, discarded `_policy_id`, `fill_timing_mae_ms = 0.0`, ×1.5/×1.2/×1.1 drawdown multipliers and forbidden bps tolerance. Added `v8-core/src/benchmark/parity.rs`: parity is computed from two manifest-declared physical ledgers under a versioned `SemanticMapping` (`v8.d153.parity.mapping.v1`), using IEEE-754 bit-pattern equality, recording `None` rather than invented zeros when a field or sequence is absent, blocking placeholder engine/method versions, deriving (never storing) non-forgeable diagnostic authority, and emitting machine-readable `reconciliation_gaps()` that state commission, funding and terminal-balance parity remain unmapped for D-116. Added `BenchmarkReceipt::with_parity()` and the `benchmark parity` CLI. Verified by 50 tests in `v8-core/tests/d153_parity_adapters_policy_bound.rs`, including source-level guards that the deleted literals cannot return.
- **#330 Governance and EN/TR reconciliation:** aligned the D-153 spec status header to the register (`RATIFIED_DECISION` was divergent), removed every phantom `benchmark_fabric_adversarial.rs` citation, committed the missing Turkish full-text mirrors for D-153 and D-159 (Rule 44 / D-149), reconciled `docs/contracts/IMPLEMENTATION_LAYOUT.md` §§1.1/2/4 for the #327–#329 modules, downgraded the unsupported D-153 "Ratified and fully completed" claim above, regenerated the EN/TR monographs with `tools/build_monograph.py` rather than hand-editing HTML, and added `tools/audit_doc_path_refs.py` as the negative path-reference guard. The guard classifies every backticked repository citation as `RESOLVED`, `RUNTIME_OUTPUT`, `HISTORICAL_RECORD`, `RETIRED`, `ROOT_DECLARED` or a per-line `ALLOWED` marker, and fails on `UNACCOUNTED`; on this branch it reports 0 unaccounted citations across the scanned corpus and its `--self-test` confirms a phantom cited as existing and an invented source file both exit non-zero.

No economic metric, p-value, effect size, tolerance, expected improvement or confidence value is introduced anywhere in this work; the fabricated vectors and multipliers are removed rather than recalibrated. Every verdict in scope remains `NO_ECONOMIC_CLAIM`, and no D-153 or D-159 artifact grants deployment or promotion authority.

Modified artifacts: `docs/contracts/D159_RESEARCH_VALIDITY_AUDIT_SPEC.md`, `docs/tr/D159_RESEARCH_VALIDITY_AUDIT_SPEC.md`, `docs/contracts/D153_BENCHMARK_FABRIC_SPEC.md`, `docs/tr/D153_BENCHMARK_FABRIC_SPEC.md`, `docs/decisions/DECISION_REGISTER.md`, `docs/tr/DECISION_REGISTER.md`, `docs/contracts/IMPLEMENTATION_LAYOUT.md`, `docs/CHANGELOG.md`, `tools/audit_doc_path_refs.py`, `tools/build_monograph.py` (D-152/D-153/D-156/D-159 specs added to the monograph corpus; corpus-selection determinism was fixed separately in `a03eb990`), `site/index.html`, `site/tr.html`, and the Rust/test surfaces listed in the #327–#329 commits on branch `audit/research-validity-20260906`.
...
> **Completion claim corrected on 2026-09-07 under D-159 (issue #330).** This entry originally opened with
> "Ratified and fully completed". That was not supported by receipts. D-153 is registered as
> `PROVISIONAL_DECISION`, not ratified; the D-156 registered benchmark-evaluator `OPEN_PIN-156-1` was and remains
> open, so no benchmark receipt can be produced from a registered data-backed evaluator. The "External Parity
> Adapters" and "Append-Only Disk Ledger" bullets below were also overstated: parity was implemented as fixed
> in-process vectors and the ledger trusted its own stored digest. Both were replaced by #328 and #329.
> The original text is preserved below with inline corrections so the historical claim stays auditable.

Implemented the D-153 Benchmark Fabric against `site/V8.5_D153_Benchmark_Fabric_Research_Monograph.html` and `docs/contracts/D153_BENCHMARK_FABRIC_SPEC.md`, at provisional (not ratified) status:
...
- **External Parity Adapters:** *[2026-09-06 claim incorrect; corrected by #329]* `external.rs` originally evaluated disagreement detection only, and its "parity" path (`evaluate_parity(policy_id)`) discarded the policy id as `_policy_id` and compared two hardcoded arrays under a bps tolerance that `PARITY_AND_IDENTITY_SPEC` forbids, with `fill_timing_mae_ms` hardcoded to `0.0` and drawdown "discrepancies" derived by multiplying PnL error by 1.5 / 1.2 / 1.1. That fabricated surface is deleted; LEAN, skfolio and VectorBT parity is now artifact-bound in `v8-core/src/benchmark/parity.rs` (see the #329 bullet in the D-159 entry above).
- **Append-Only Disk Ledger:** *[2026-09-06 claim overstated; corrected by #328]* Implemented a hash-chain ledger with disk persistence (`.audit/benchmark/ledger.jsonl`), but the chain bound the *stored* digest rather than canonical receipt bytes, so fields outside the digest list could change undetected. Superseded by the `d153.ledger.v2` entry seal (see the #328 bullet in the D-159 entry above).
...
> Note: the register and this log also cited `v8-core/tests/benchmark_fabric_adversarial.rs`, a file that never existed; the sabotage suite is `v8-core/tests/d153_benchmark_fabric_sabotage.rs`. All phantom citations are removed under D-159. The Turkish full-text mirror `docs/tr/D153_BENCHMARK_FABRIC_SPEC.md` was missing at this date and is committed under D-159.
