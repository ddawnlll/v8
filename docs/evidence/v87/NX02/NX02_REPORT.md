...
|---|---|---|---|
| R1 | `evaluation/runner.py` artık `economic_benchmark.pair_positions` tabanlı `campaign_accounting` kullanır (ilk-id eşleşmesi kaldırıldı); `evaluation/economic_benchmark.py` ortak sözleşmeyi barındırır | `pytest -q v8-next/tests/test_runner_accounting_nx02.py` | PASS — tekrar kullanılan `position_id` altında eşleşme kronolojik: `(5.0, -3.0)`; eski davranış `(5.0, 5.0)` verirdi |
