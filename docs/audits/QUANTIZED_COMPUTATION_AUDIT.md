...
`PROJECT_EVIDENCE_SUPPORTED`: after the D-118 remediation below, the Rust
release handoff suite passed 312/312 tests on 2026-08-20, including scalar/SIMD
...
## 4. Adjacent repository defect discovered by the audit

`PROJECT_EVIDENCE_SUPPORTED`: the initial audit found a second, CLI-unwired
`EvaluationEngine` alongside the registered `statistics/` and `report.rs`
authority path. The orphan engine could emit `SUPPORTED_EDGE` without an
authority receipt and wrote placeholder/fabricated provenance values, including
count-derived tape/config identifiers, a fixed timestamp, fixed artifact
size/file count, and fixed multiple-testing diagnostics. Its duplicate
statistics implementation also had panic surfaces for zero resample counts and
non-finite values.

`PROJECT_EVIDENCE_SUPPORTED`: before D-118 landed, upstream independently
closed the unauthorized verdict and artifact-inventory defects. The engine
nevertheless remained unwired, with count-derived tape/config identifiers, a
fixed timestamp and trial count, and a duplicate panic-prone statistics path.
D-118 / issue #199 removed that engine and its private helpers. The support
types used by Target Oracle coverage and the shared `TradeRow` used by Kaizen
remain; they have no autonomous verdict or economic authority.
...
backend parity, and a registry decision. D-118 removed the orphan evaluation
engine instead of repairing or promoting a second verdict architecture.
