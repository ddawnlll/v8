...
def main(argv: list[str] | None = None) -> int:
    ...
    for policy_id in [name.strip() for name in args.policies.split(",") if name.strip()]:
        ...
        for value in nets:
            ...
            max_dd = max(max_dd, (peak - equity) if peak > 0 else 0.0)
        metrics = metrics_from_campaigns(
            ...
            max_drawdown_pct=max_dd * 100.0, bars=len(series),
        )
        ...
        report["families"][policy_id] = {
            ...
            "net_with_modelled_slippage_10_sum": round(
                sum(row["net_with_modelled_slippage_10"] for row in trades), 8
            ),
