...
def test_g8_live_realization_modes(tmp_path: Path):
    ...
    state_unmatched, metrics_unmatched = evaluate_g8_live_realization(
        live_fills_path=fills_file,
        account={"balance_total": "100", "positions": [], "orders": [], "currency": "USDT"},
    )
    assert state_unmatched == GateState.PASS
    assert metrics_unmatched["account_reconciled"] is False
    assert metrics_unmatched["reconciliation"]["status"] == "EXECUTION_UNPROVEN"
