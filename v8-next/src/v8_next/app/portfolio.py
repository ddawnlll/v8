...
from v8_next.adapters.expert_strategy import SignalCache
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        action="store_true",
        default=True,
        help="Retain the full per-bar 28-expert stance trace (enabled by default).",
    )
    p.add_argument(
        "--fast",
        action="store_true",
        help="Run one cached engine pass only; skips receipt/statistical verification.",
    )
...
def build_shadow_section(
    ...
    if fills:
        ...
        section["shortfall"] = decompose_fill_shortfall(fills)
        section["latency"] = summarize_fill_latency(fills)
    return section
...
def capacity_from_participation(
    ...
    No impact coefficient is produced here.  A bar-volume participation bound
    is not an L2/tick capacity calibration; impact remains absent until
    ``evaluation.capacity`` receives real sequenced microstructure observations.
        ...
        out.append(
            {
                ...
                "model": "IMPACT_REQUIRES_MICROSTRUCTURE_CAPTURE",
                "k": k,
                "k_status": "UNSUPPORTED_NO_VENUE_CALIBRATION",
                "sigma_per_bar": None,
                "impact_bps_at_peak": None,
...
def run_fast_diagnostic(args: argparse.Namespace, tape: MultiTape, out_dir: Path) -> int:
    """Run one cached engine pass without receipt/statistical verification.

    This is an execution hot-path measurement, not an economic benchmark. The
    canonical five-run receipt chain remains available through the default path.
    """
    ...
    payload = {
        ...
        "opened_positions": len(result["opened_positions"]),
        "closed_positions": len(result["closed_positions"]),
        "account": result["account"],
        "funding_coverage": result["funding_coverage"],
    }
    output = out_dir / f"fast_diagnostic_{tape.tape_sha256[:12]}.json"
    output.write_text(json.dumps(payload, indent=2, default=str), encoding="utf-8")
    print(
        f"[+] FAST_DIAGNOSTIC_ONLY: {elapsed_ms:.1f} ms, "
        f"{result['total_bars']} bars, {len(result['opened_positions'])} trades"
    )
    print(f"[+] Fast diagnostic artifact: {output}")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.fast:
        return run_fast_diagnostic(args, tape, out_dir)
    ...
    signal_cache = SignalCache()
    kw["signal_cache"] = signal_cache
    # Canonical runs always retain the complete 28-expert trace.  The cache
    # removes repeated feature/stance work across verification legs; audit
    # completeness must not be traded away for the speedup.
    kw["audit_trace"] = True
