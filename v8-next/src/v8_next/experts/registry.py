...
def observe_expert(
    ...
    *,
    ...
    # Authoritatively wrap and apply canonical witness metadata. Construct the
    # frozen value directly instead of dataclasses.replace(), which deep-copies
    # the dataclass field map for every expert on every bar.
    return Stance(
        observer_id=spec.expert_id,
        dependency_group=spec.dependency_group,
        kind=raw_stance.kind,
        reason=raw_stance.reason,
        opportunity_id=raw_stance.opportunity_id,
        decision_ns=raw_stance.decision_ns,
...
def observe_all_28(
    ...
    *,
    readings: tuple[PositioningReading, ...] = (),
    fast_no_opportunity: bool = False,
) -> tuple[Stance, ...]:
    """Observe an opportunity across all 28 canonical active experts with authoritative metadata."""
    if fast_no_opportunity and opportunity is None:
        # Every active witness is causally abstaining when no episode exists.
        # Preserve the complete typed 28-row shape and registry metadata while
        # avoiding feature work that cannot affect consensus or orders.
        return tuple(
            Stance(
                observer_id=spec.expert_id,
                dependency_group=spec.dependency_group,
                ...
                mechanism_family=spec.mechanism_family,
                version=f"{spec.expert_id}-witness-{spec.version}",
                variant_id=spec.default_variant or "UNRESOLVED",
            )
            for spec in (EXPERT_REGISTRY[spec_id] for spec_id in CANONICAL_28_EXPERTS)
        )
    return tuple(
