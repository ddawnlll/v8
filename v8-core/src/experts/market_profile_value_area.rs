...
fn tpo_profile(
    ...
    let max_high = prior_bars
        .iter()
        .map(|b| b.high)
        .fold(f64::NEG_INFINITY, f64::max);
    let min_low = prior_bars
        .iter()
        .map(|b| b.low)
    ...
    let above: i64 = counts
        .iter()
        .filter(|(k, _)| **k > poc_idx)
        .map(|(_, v)| *v)
        .sum();
    let below: i64 = counts
        .iter()
        .filter(|(k, _)| **k < poc_idx)
        .map(|(_, v)| *v)
        .sum();
    Some((
        poc_idx as f64 * bucket,
        lo_i as f64 * bucket,
        hi_i as f64 * bucket,
        total,
...
pub fn market_profile_value_area(fm: &FeatMap, expert_id: &str, version: &str) -> ExpertEval {
    ...
    let bof = match fm.value("bar_of_session") {
        Some(v) => v,
        None => return no_habitat(expert_id, version, fm.as_of),
    };
    ...
    let day_high = prior_day
        .iter()
        .map(|b| b.high)
        .fold(f64::NEG_INFINITY, f64::max);
    let day_low = prior_day
        .iter()
        .map(|b| b.low)
        .fold(f64::INFINITY, f64::min);
