...
/// Explicit historical diagnostic; returns a report without writing to the repository.
/// Reads real tape data and fails on missing, invalid or empty input.
pub fn conflict_source_decomposition(tape_path: &std::path::Path) -> Result<String, String> {
    let rows = crate::runloop::read_tape(&tape_path.to_path_buf())?;
    let ds = crate::data::Dataset::from_rows(rows).map_err(|e| e.to_string())?;
    let stores = crate::state::build_stores(&ds);
    let store = stores.first().ok_or("diagnostic tape has no price bars")?;
    ...
    let projections: Vec<_> = loop_engine
        ...
        .map(|w| {
            let closure =
                crate::features::group_closure(crate::experts::requires_for(&w.expert_id));
            let allows_hist = crate::features::history_allowed(&closure);
    ...
    for bar_idx in 32..n_bars {
        ...
        let hist = crate::state::history_bars(store, t, 32);
        let empty_variants = HashMap::new();
        let detected_episodes = loop_engine
            .grammar
            .scan_market_state(symbol, venue, store, bar_idx, &loop_engine.resolver)
            .map_err(|e| format!("market grammar: {e:?}"))?;

        for dir in [
            crate::opportunity::exposure::ExposureDirection::Long,
            crate::opportunity::exposure::ExposureDirection::Short,
        ] {
            let matching_ep = detected_episodes
                .iter()
                .find(|e| e.exposure.direction == dir);

            if let Some(ep) = matching_ep {
                let mut evidences = Vec::with_capacity(loop_engine.witnesses.len());
                for (witness, (_, closure, allows_hist)) in
                    loop_engine.witnesses.iter().zip(&projections)
                {
                    let expert_hist = if *allows_hist { hist.as_slice() } else { &[] };
                    let fm = crate::experts::base::FeatMap {
                        features: crate::experts::base::ProjectedFeatures::new(&feats, closure),
                ...
                if active_supports > 0 {
                    ...
                    if let Ok(rec) =
                        crate::opportunity::reconcile::EvidenceReconciler::reconcile(ep, &evidences)
                    {
                        ...
                        if rec.aggregate_stance
                            != crate::opportunity::reconcile::ReconciledStance::Supported
                        {
                            ...
                            for ev in &evidences {
                                if ev.is_active_support() {
                                    *support_counts.entry(ev.observer_id.clone()).or_insert(0) += 1;
                                    ...
                                    ev.stance,
                                    crate::opportunity::evidence::ObserverStance::Contradict { .. }
                                ) {
                                    *contradict_counts
                                        .entry(ev.observer_id.clone())
                                        .or_insert(0) += 1;
    ...
    report_out.push_str(&format!(
        "Total Stage 3 evaluations: {total_stage3_evals}\n"
    ));
    report_out.push_str(&format!(
        "Total Stage 3 drops (Contradiction/Entropy): {total_stage3_drops}\n"
    ));
    ...
    top_contradicters.sort_by(|a, b| b.1.cmp(&a.1).then_with(|| a.0.cmp(&b.0)));
    ...
    top_supporters.sort_by(|a, b| b.1.cmp(&a.1).then_with(|| a.0.cmp(&b.0)));
    ...
    for ((s, c), count) in top_pairs.iter().take(20) {
        report_out.push_str(&format!(
            "  - Supporter: {:<30} vs Contradicter: {:<30} -> {} occurrences\n",
            s, c, count
        ));
    }
    report_out.push_str("================================================\n");
